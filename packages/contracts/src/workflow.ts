...
export const CreateArtifactVersionInputSchema = z.object({
  ...
  inlineText: z.string().trim().min(1).max(100_000),
  sha256: z.string().regex(/^[a-f0-9]{64}$/),
  clientRequestId: z.string().uuid()
}).strict();

export const RightsAttestationInputSchema = z.object({
  artifactId: z.string().uuid(),
  statementVersion: z.string().trim().min(1).max(64),
  clientRequestId: z.string().uuid()
}).strict();

export const TransitionInputSchema = z.object({
  expectedStage: WorkflowStageSchema,
...
export const CreateConsentProfileInputSchema = z.object({
  kind: IdentityProfileKindSchema,
  displayName: z.string().trim().min(1).max(120),
  consentStatementVersion: z.string().trim().min(1).max(64),
  consentSha256: z.string().regex(/^[a-f0-9]{64}$/),
  clientRequestId: z.string().uuid()
}).strict();

export const RegisterIdentitySampleInputSchema = z.object({
...
export const RevokeConsentProfileInputSchema = z.object({
  reason: z.string().trim().min(1).max(1000),
  idempotencyKey: z.string().uuid()
}).strict();

export const DeleteConsentProfileInputSchema = z.object({
  idempotencyKey: z.string().uuid()
}).strict();

export type ConsentEligibility = Readonly<{
  revoked: boolean;
  samples: readonly Readonly<{ status: SampleStatus }>[];
}>;

export function isIdentityProfileGenerationEligible(input: ConsentEligibility): boolean {
  return !input.revoked && input.samples.some(sample => sample.status === 'validated');
}
